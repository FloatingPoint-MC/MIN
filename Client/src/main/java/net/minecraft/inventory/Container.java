    ...
    public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, EntityPlayer player) {
        ...
        if (clickTypeIn == ClickType.QUICK_CRAFT) {
            ...
            if (slot4.canTakeStack(player)) {
                ItemStack itemStack3 = inventoryplayer.getStackInSlot(dragType);
                boolean flag = itemStack3.isEmpty() || slot4.inventory == inventoryplayer && slot4.isItemValid(itemStack3);
                int k1 = -1;
                ...
                if (slot4.getHasStack() && flag) {
                    ItemStack itemStack4 = slot4.getStack();
                    inventoryplayer.setInventorySlotContents(dragType, itemStack4.copy());

                    if ((slot4.inventory != inventoryplayer || !slot4.isItemValid(itemStack3)) && !itemStack3.isEmpty()) {
                        if (k1 > -1) {
                            inventoryplayer.addItemStackToInventory(itemStack3);
                            slot4.decrStackSize(itemStack4.getCount());
                            slot4.putStack(ItemStack.EMPTY);
                            slot4.onTake(player, itemStack4);
                        }
                    } else {
                        slot4.decrStackSize(itemStack4.getCount());
                        slot4.putStack(itemStack3);
                        slot4.onTake(player, itemStack4);
                    }
                } else if (!slot4.getHasStack() && !itemStack3.isEmpty() && slot4.isItemValid(itemStack3)) {
