...
public class CustomGuiProperties {
    ...
    private static IWorldNameable getWorldNameable(GuiScreen screen) {
        if (screen instanceof GuiBeacon) {
            return ((GuiBeacon) screen).getTileBeacon();
        } else if (screen instanceof GuiBrewingStand) {
            return ((GuiBrewingStand) screen).getTileBrewingStand();
        } else if (screen instanceof GuiChest) {
            return ((GuiChest) screen).getLowerChestInventory();
            ...
            return ((GuiEnchantment) screen).getNameable();
        } else if (screen instanceof GuiFurnace) {
            return ((GuiFurnace) screen).getTileFurnace();
        } else if (screen instanceof GuiHopper) {
            return ((GuiHopper) screen).getHopperInventory();
        } else {
            return screen instanceof GuiShulkerBox ? ((GuiShulkerBox) screen).getInventory() : null;
        }
    }
