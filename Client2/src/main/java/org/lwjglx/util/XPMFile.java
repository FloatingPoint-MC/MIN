...
public class XPMFile {
    ...
    /**
     * Given a line defining a color/pixel, parses this into an array containing a key and a color
     *
    ...
    /*
     * private static void showResult(byte[] bytes) { final BufferedImage i = new BufferedImage(16, 16,
     * BufferedImage.TYPE_INT_ARGB); int c = 0; for (int y = 0; y < 16; y++) { for (int x = 0; x < 16; x++) {
     * i.setRGB(x, y, (bytes[c] << 16) + (bytes[c + 1] << 8) + (bytes[c + 2] << 0) + (bytes[c + 3] <<
     * 24));//+(128<<24));// c += 4; } } final Frame frame = new Frame("XPM Result"); frame.add(new Canvas() { public
     * void paint(Graphics g) { g.drawImage(i, 0, 0, frame); } }); frame.addWindowListener(new WindowAdapter() { public
     * void windowClosing(WindowEvent e) { frame.dispose(); } }); frame.setSize(100, 100); frame.setVisible(true); }
     */
}
