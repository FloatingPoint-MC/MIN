...
import cn.floatingpoint.min.system.module.impl.render.impl.AttackParticles;
...
public class PlayerControllerMP {
        ...
        if (!this.currentGameType.isCreative() || this.mc.player.getHeldItemMainhand().isEmpty() || !(this.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword)) {
            ...
            if ((!(block instanceof BlockCommandBlock) && !(block instanceof BlockStructure)) || this.mc.player.canUseCommandBlock()) {
                if (blockState.getMaterial() != Material.AIR) {
                    ...
                    boolean flag = world.setBlockState(pos, Objects.requireNonNull(Blocks.AIR).getDefaultState(), 11);
                    ...
                    if (!this.currentGameType.isCreative()) {
                        ...
                        if (!itemStack.isEmpty()) {
                            itemStack.onBlockDestroyed(world, blockState, pos, this.mc.player);
    ...
    public void attackEntity(EntityPlayer playerIn, Entity targetEntity) {
        ...
        if (AttackParticles.moreCritParticle.getValue()) {
            playerIn.onCriticalHit(targetEntity);
        }
        if (AttackParticles.moreSharpnessParticle.getValue()) {
    ...
    public void windowClick(int windowId, int slotId, int mouseButton, ClickType type, EntityPlayer player) {
        short short1 = player.openContainer.getNextTransactionID();
    ...
    public void func_194338_a(int p_194338_1_, IRecipe p_194338_2_, boolean p_194338_3_) {
        this.connection.sendPacket(new CPacketPlaceRecipe(p_194338_1_, p_194338_2_, p_194338_3_));
    }

    /**
     * GuiEnchantment uses this during multiplayer to tell PlayerControllerMP to send a packet indicating the
     * enchantment actions the player has taken.
     */
