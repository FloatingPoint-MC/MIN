...
public class GuiNewChat extends Gui {
    ...
    public void drawChat(int updateCounter) {
        if (this.mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN) {
            ...
            if (j > 0) {
                ...
                fontRenderer = ChatBar.font.isCurrentMode("Minecraft") ? mc.fontRenderer : Managers.fontManager.sourceHansSansCN_Regular_18;
                for (int i1 = 0; i1 + this.scrollPos < this.drawnChatLines.size() && i1 < i; ++i1) {
                    ...
                    if (chatline != null) {
                        ...
                        if (j1 < 200 || flag) {
                            ...
                            if (l1 > 3) {
                                int j2 = -i1 * 9;
                                if (ChatBar.background.getValue()) {
                                ...
                                if (ChatBar.shadow.getValue()) {
                                    ...
                                    fontRenderer.drawString(s, 0, j2 - 8, 16777215 + (l1 << 24));
                                }
    ...
    private void setChatLine(ITextComponent chatComponent, int chatLineId, int updateCounter, boolean displayOnly) {
        ...
        List<ITextComponent> list = GuiUtilRenderComponents.splitText(chatComponent, i, fontRenderer, false, false);
        boolean flag = this.getChatOpen();
    ...
    public ITextComponent getChatComponent(int mouseX, int mouseY) {
        if (this.getChatOpen()) {
            ...
            if (j >= 0 && k >= 0) {
                ...
                if (j <= MathHelper.floor((float) this.getChatWidth() / this.getChatScale()) && k < fontRenderer.FONT_HEIGHT * l + l) {
                    ...
                    if (i1 >= 0 && i1 < this.drawnChatLines.size()) {
                        ...
                        for (ITextComponent itextcomponent : chatline.getChatComponent()) {
                            if (itextcomponent instanceof TextComponentString) {
                                j1 += fontRenderer.getStringWidth(GuiUtilRenderComponents.removeTextColorsIfConfigured(((TextComponentString) itextcomponent).getText(), false));
