 ...
 * 
 * @author $Author: spasi $
 * @version $Revision: 3418 $ $Id: Display.java 3418 2010-09-28 21:11:35Z spasi $
 */
public final class Display {
    ...
    /**
     * Determine the available display modes that match the specified minimum and maximum criteria. If any given
    ...
    public static DisplayMode[] getAvailableDisplayModes(int minWidth, int minHeight, int maxWidth, int maxHeight,
        ...
        for (int i = 0; i < modes.length; i++) {
            ...
            if (maxHeight != -1 && modes[i].getHeight() > maxHeight) continue;
            if (minBPP != -1 && modes[i].getBitsPerPixel() < minBPP) continue;
            if (maxBPP != -1 && modes[i].getBitsPerPixel() > maxBPP) continue;
            // if (modes[i].bpp == 24)
            // continue;
            if (modes[i].getFrequency() != 0) {
                if (minFreq != -1 && modes[i].getFrequency() < minFreq) continue;
                if (maxFreq != -1 && modes[i].getFrequency() > maxFreq) continue;
            }
            matches.add(modes[i]);
        }
    ...
    /**
     * Create the display by choosing from a list of display modes based on an order of preference. You must supply a
     * list of allowable display modes, probably by calling getAvailableDisplayModes(), and an array with the order in
            ...
            public int compare(DisplayMode dm1, DisplayMode dm2) {
                for (FieldAccessor accessor : accessors) {
                    ...
                    if (accessor.usePreferred && f1 != f2) {
                        if (f1 == accessor.preferred) return -1;
                        else if (f2 == accessor.preferred) return 1;
                        else {
