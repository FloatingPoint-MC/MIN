...
public abstract class Entity implements ICommandSender {
    ...
    /**
     * If true, an {@link net.minecraft.network.play.server.SPacketEntityVelocity} will be sent updating this entity's velocity.
     */
    ...
    public void move(MoverType type, double x, double y, double z) {
        if (this.noClip) {
            ...
            if ((type == MoverType.SELF || type == MoverType.PLAYER) && this.onGround && this.isSneaking() && this instanceof EntityPlayer) {
                while (x != 0.0D && this.world.getCollisionBoxes(this, this.getEntityBoundingBox().offset(x, -this.stepHeight, 0.0D)).isEmpty()) {
                    if (x < 0.05D && x >= -0.05D) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= 0.05D;
    ...
    /**
     * Plays the {@link #getSplashSound() splash sound}, and the {@link EnumParticleTypes#WATER_BUBBLE} and {@link
     * EnumParticleTypes#WATER_SPLASH} particles.
     */
    ...
    public void readFromNBT(NBTTagCompound compound) {
        try {
            ...
            if (compound.hasUniqueId("UUID")) {
                ...
                this.cachedUniqueIdString = Objects.requireNonNull(this.entityUniqueID).toString();
            }
    ...
    public void updateRidden() {
        ...
        if (this.isRiding() && Objects.requireNonNull(entity).isDead) {
            this.dismountRidingEntity();
        } else {
            this.motionX = 0.0D;
            this.motionY = 0.0D;
            ...
            if (this.isRiding()) {
                Objects.requireNonNull(entity).updatePassenger(this);
            }
        }
    }
    ...
    public boolean isVisibleToPlayer(EntityPlayer player) {
        if (player.isSpectator()) {
            ...
            return team != null && player.getTeam() == team && team.getSeeFriendlyInvisiblesEnabled() || !this.isInvisible();
        }
    }
     ...
     *
     * <dl>
     * <dt>{@link net.minecraft.world.IWorldNameable#getName() INameable.getName()}</dt>
     * <dd>Returns the name of this inventory. If this {@linkplain net.minecraft.inventory.IInventory#hasCustomName() has a custom
     ...
     * <dt>{@link ICommandSender#getName() ICommandSender.getName()}</dt>
    ...
    public Entity changeDimension(int dimensionIn) {
        if (!this.world.isRemote && !this.isDead) {
            ...
            WorldServer worldserver = Objects.requireNonNull(minecraftserver).getWorld(i);
    ...
    public void addEntityCrashInfo(CrashReportCategory category) {
        ...
        category.addDetail("Entity's Vehicle", () -> Entity.this.getRidingEntity() != null ? Entity.this.getRidingEntity().toString() : "null");
    }
     ...
     *
     * <dl>
     * <dt>{@link net.minecraft.world.IWorldNameable#getDisplayName() INameable.getDisplayName()}</dt>
     * <dd>A normal component. Might be a translation component or a text component depending on the context. Usually
     * implemented as:</dd>
     * <dd><pre><code>return this.{@link net.minecraft.world.IWorldNameable#hasCustomName() hasCustomName()} ? new
     * TextComponentString(this.{@link #getName()}) : new TextComponentTranslation(this.{@link
     * #getName()});</code></pre></dd>
     * <dt>{@link ICommandSender#getDisplayName() ICommandSender.getDisplayName()}</dt>
     * <dd>For most entities, this returns the result of {@link #getName()}, with {@linkplain
     * ScorePlayerTeam#formatPlayerName scoreboard formatting} and a {@linkplain
     * Entity#getHoverEvent special hover event}.</dd>
     ...
     *
     * <dl>
     * <dt>{@link net.minecraft.world.IWorldNameable#hasCustomName() INameable.hasCustomName()}</dt>
