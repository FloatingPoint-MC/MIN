...
public class StringCache {
    ...
    /**
     * If true, then enable GL_BLEND in renderString() so antialiasing font glyphs show up properly.
     */
     ...
     */
    private int sizeString(String str, int width) {
        /* Check for invalid arguments */
        if (str == null || str.isEmpty()) {
            return 0;
        }
    ...
    public String trimStringToWidth(String str, int width, boolean reverse) {
        int length = sizeString(str, width);
    ...
    private int applyColorCode(int colorCode, int color, boolean shadowFlag) {
        ...
        Tessellator.getInstance().getBuffer().color(color >> 16 & 0xff, color >> 8 & 0xff, color & 0xff, color >> 24 & 0xff);
        return color;
    }
    ...
    private int stripColorCodes(Entry cacheEntry, String str, char[] text) {
        ...
        while ((next = str.indexOf('\247', start)) != -1 && next + 1 < str.length()) {
            /*
