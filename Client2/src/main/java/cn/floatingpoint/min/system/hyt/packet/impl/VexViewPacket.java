...
public class VexViewPacket implements CustomPacket {
    ...
    public void process(ByteBuf byteBuf) {
        ...
        if (buttonDecoder.invited) {
            ...
            textComponents.getStyle().setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/min hyt party handle " + buttonDecoder.getButton("accept").getId() + " " + buttonDecoder.getButton("deny").getId()));
