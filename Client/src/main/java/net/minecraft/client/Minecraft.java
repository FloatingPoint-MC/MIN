...
public class Minecraft implements IThreadListener, ISnooperInfo {
    ...
    private void populateSearchTreeManager() {
        SearchTree<ItemStack> searchtree = new SearchTree<>((p_193988_0_) ->
                p_193988_0_.getTooltip(null, ITooltipFlag.TooltipFlags.NORMAL).stream().map(TextFormatting::getTextWithoutFormattingCodes).filter(Objects::nonNull).map(String::trim).filter((p_193984_0_) -> !p_193984_0_.isEmpty()).collect(Collectors.toList()), (p_193985_0_) ->
        ...
        SearchTree<RecipeList> searchtree1 = new SearchTree<>((p_193990_0_) ->
                p_193990_0_.getRecipes().stream().flatMap((p_193993_0_) -> p_193993_0_.getRecipeOutput().getTooltip(null, ITooltipFlag.TooltipFlags.NORMAL).stream()).map(TextFormatting::getTextWithoutFormattingCodes).filter(Objects::nonNull).map(String::trim).filter((p_193994_0_) -> !p_193994_0_.isEmpty()).collect(Collectors.toList()), (p_193991_0_) ->
    ...
    private void setWindowIcon() {
        ...
        if (util$enumos != Util.EnumOS.OSX) {
            ...
            try {
                inputstream = this.defaultResourcePack.getResourceStream(new ResourceLocation("min/icon_16x16.png"));
                inputstream1 = this.defaultResourcePack.getResourceStream(new ResourceLocation("min/icon_32x32.png"));
    ...
    private void runGameLoop() throws IOException {
        ...
        ++this.fpsCounter;
        boolean flag = this.isSingleplayer() && this.currentScreen != null && this.currentScreen.doesGuiPauseGame() && !Objects.requireNonNull(this.integratedServer).getPublic();
    ...
    public void displayInGameMenu() {
        if (this.currentScreen == null) {
            ...
            if (this.isSingleplayer()) {
                assert this.integratedServer != null;
    ...
    public void runTick() throws IOException {
        ...
        if (this.currentScreen != null) {
            try {
                ...
                crashreportcategory.addDetail("Screen name", () -> Minecraft.this.currentScreen.getClass().getCanonicalName());
            ...
            if (this.currentScreen != null) {
                try {
                    ...
                    crashreportcategory1.addDetail("Screen name", () -> Minecraft.this.currentScreen.getClass().getCanonicalName());
    ...
    private void debugFeedbackTranslated(String untranslatedTemplate, Object... objs) {
        this.ingameGUI.getChatGUI().printChatMessage((new TextComponentString("")).appendSibling((new TextComponentTranslation("debug.prefix")).setStyle((new Style()).setColor(TextFormatting.YELLOW).setBold(Boolean.TRUE))).appendText(" ").appendSibling(new TextComponentTranslation(untranslatedTemplate, objs)));
    }
    ...
    public void setDimensionAndSpawnPlayer(int dimension) {
        ...
        this.player.getDataManager().setEntryValues(Objects.requireNonNull(entityplayersp.getDataManager().getAll()));
    ...
    public void addServerTypeToSnooper(Snooper playerSnooper) {
        ...
        if (gameprofile.getId() != null) {
            playerSnooper.addStatToSnooper("uuid", Hashing.sha1().hashBytes(gameprofile.getId().toString().getBytes(Charsets.ISO_8859_1)).asBytes());
        }
    }
